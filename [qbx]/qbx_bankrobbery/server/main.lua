...
RegisterNetEvent('qbx_bankrobbery:server:SetStationStatus', function(key, isHit)
    ...
    if allStationsHit() then
        -- exports['qbx_gametime']:setBlackout(true) -- Отключено: функция не поддерживается в qbx_gametime
        ...
        CreateThread(function()
            SetTimeout(60000 * config.blackoutTimer, function()
                -- exports['qbx_gametime']:setBlackout(false) -- Отключено: функция не поддерживается в qbx_gametime
